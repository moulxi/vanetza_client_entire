...
namespace vanetza
{
    namespace security
    {
        ...
        {
            Issuer issuer;
            std::list<toBeSigned> tobesigned;
            Signature signature;
             ...
             */
            const toBeSigned *get_attribute(toBeSignedType type) const;

            /**
             * Get validity restriction of a certain type (if present)
             * \param type of validity restriction
             */
            // const toBeSigned *get_restriction(toBeSignedType type) const;

            /**
             * Remove subject attribute of a certain type (if present)
             * \param type of subject attribute
             */
            void remove_attribute(toBeSignedType type);
            ...
            const tobesigned_type<T> *get_attribute() const
            {
                using type = tobesigned_type<T>;
                const toBeSigned *field = get_attribute(T);
