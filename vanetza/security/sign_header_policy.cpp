...
namespace vanetza
{
    namespace security
    {
        ...
        DefaultSignHeaderPolicy::DefaultSignHeaderPolicy(const Runtime &rt, PositionProvider &positioning) : m_runtime(rt), m_positioning(positioning), m_cam_next_certificate(m_runtime.now()), m_cert_requested(false), m_chain_requested(false)
        {
        }
        ...
        std::list<HeaderField> DefaultSignHeaderPolicy::prepare_header(const SignRequest &request, CertificateProvider &certificate_provider)
        {
            ...
            if (request.its_aid == aid::CA)
            {
                ...
                if (m_chain_requested)
                {
                    ...
                    header_fields.push_back(Issuer{std::move(full_chain)});
                    m_cam_next_certificate = m_runtime.now() + std::chrono::seconds(1);
                }
                else if (m_runtime.now() < m_cam_next_certificate && !m_cert_requested)
                {
                    header_fields.push_back(Issuer{calculate_hash(certificate_provider.own_certificate())});
                }
                else
                {
                    header_fields.push_back(Issuer{certificate_provider.own_certificate()});
                    m_cam_next_certificate = m_runtime.now() + std::chrono::seconds(1);
                }
