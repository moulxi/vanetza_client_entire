        ...
        if (vm.count("certificate") && vm.count("certificate-key"))
        {
            ...
            if (vm.count("certificate-chain"))
            {
                for (auto &chain_path : vm["certificate-chain"].as<std::vector<std::string>>())
                {
                    ...
                    std::string id_name;
                    // Get name
                    for (auto &subject_attr : chain_certificate.tobesigned)
                    {
                        security::toBeSignedType attr_type = get_type(subject_attr);
                        if (attr_type == security::toBeSignedType::Name_id)
                        {
                            security::NameId id = boost::get<security::NameId>(subject_attr);
                            if (id.name.size() > 0)
                            {
...
{
    options.add_options()("security", po::value<std::string>()->default_value("dummy"), "Security entity [none,dummy,certs]")("certificate", po::value<std::string>(), "Certificate to use for secured messages.")("certificate-key", po::value<std::string>(), "Certificate key to use for secured messages.")("certificate-chain", po::value<std::vector<std::string>>()->multitoken(), "Certificate chain to use, use as often as needed.")("trusted-certificate", po::value<std::vector<std::string>>()->multitoken(), "Trusted certificate, use as often as needed. Root certificates in the chain are automatically trusted.");
}
